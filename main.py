...
async def on_message(message):
    ...
    if message.content.startswith("?Version") or message.content.startswith("?version"):        
        msg = "{0.author.mention} 도리봇은 2021년 12월 05일에 1.12.11 버전으로 업데이트되었어요. 나머지 내용들은 DM을 확인해주세요. 도리봇이 DM을 보내지 않는다면, 한번 더 챗을 치세요.".format(message)
    ...
    if message.content == "?Version" or message.content.startswith("?version"):
        if message.author.dm_channel:
            ...
            await message.author.dm_channel.send("제 버전은 1.12.11으로 가장 최신버전이에요.")
            await message.author.dm_channel.send("도리봇의 금칙어 필터링 기능이 개선됩니다.") 
            await message.author.dm_channel.send("본문 중간에 삽입된 단어들마저도 ") 
            await message.author.dm_channel.send("이제 감지해낼수 있습니다. ")             
            await message.author.dm_channel.send("###############################")            
            await message.author.dm_channel.send("Would recommend to you.")
            await message.author.dm_channel.send("Say ?Corona")
    ...
    if message.content.startswith("?코로나") or message.content.startswith("?Corona"):
     msg = await message.channel.send("코로나 종식 선언!이라고 할줄 알았냐?")
     await asyncio.sleep(4.0)
     await msg.edit(content="2021년 11월 1일부터 위드코로나가 실시됩니다만 한달만에 다시 거리두기체제로 돌아갑니다.")  
    ...
    if message.content.startswith("?hello") or message.content.startswith("?안녕"):
        msg = await message.channel.send("안뇽안뇽!!")

             
    if message.content.startswith("ㅋ") or message.content.startswith("ㅎ") or message.content.startswith("하하") or message.content.startswith("히히"):         
        dtime = datetime.datetime.now()
    ...
    if message.content.startswith("7호선"):
        await message.channel.send("해당 닉네임의 소유자는 하층민이죠, 언급할 자격이 충분이 있어보입니다.")  
        await message.channel.send("해당 닉네임의 소유자는 현재 전라도에 있는 신안보육원에 계십니다.")
        await message.channel.send("주소: 전라남도 신안군 압해읍 동서리 808-11") 
